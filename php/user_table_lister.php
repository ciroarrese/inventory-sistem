<?php
    ...
    /**
     * Función encargada de retornar la página del listado actual y la url en la que nos encoentramos.
     * 
     * @param string $vista la vista en donde se realiará la paginación.
     * 
     * @return array retorna un array de datos compuesto por los clave -> valor: 
     * 'page' -> valor de la paginción actual, 
     * 'url' -> valor del url actual.
     */
    function tablePages($vista){
        if (!isset($_GET['page'])) {
            $page = 1;
        } else {
            $page = (int) $_GET['page'];
        ...
        $url = 'index.php?vista=' . $vista . '&page=';
    ...
    /**
     * Funcion encargada de mostrar una tabla de datos compuesta por los usuarios que se encuentran en la BD
     * 
     * @param string $page contine el numero de la pagina actual
     * @param string $url contiene la url actual
     * @param int $records cantidad de registros a mosotrar por pagina
     * @param string $search en caso de ser distinto a '' contiene un valor a buscar en la base de datos
     * 
     * @return void 
     */
    function table($page, $url, $records, $search){
